...
import { saveQuizAttemptAction } from "@/app/dashboard/quiz/actions";
...
export function useQuizEngine() {
  ...
  const endQuiz = useCallback(
    async (options?: QuizEndOptions) => {
      ...
      try {
        const attempt = await saveQuizAttemptAction(payload);
  ...
  endQuizRef.current = (options) => {
    void endQuiz(options);
  };
  ...
  const startQuiz = useCallback(async (setup: QuizSetupConfig) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          (err as { error?: string }).error ?? "Failed to generate quiz"
        );
      }
